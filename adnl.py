...
__copyright__ = "Copyright (c) 2023, SaluteDevices"
__version__ = '0.0.1'
...
def run_bootrom_stage(epout, epin, aml_img):
    ...
    send_burnsteps(epout, epin, [0x0, 0x0, 0x4, 0xc0])
    send_cmd(epout, epin, 'getvar:getchipinfo-1')
    send_burnsteps(epout, epin, [0x1, 0x0, 0x4, 0xc0])
    ...
    send_burnsteps(epout, epin, [0x2, 0x0, 0x4, 0xc0])
...
def run_bl2_stage(epout, epin, aml_img):
    ...
    send_burnsteps(epout, epin, [0x3, 0x0, 0x4, 0xc0])

    item = aml_img.item_get('USB', 'UBOOT')
    rest = item.size()

    while True:
        to_send = min(USB_BULK_SIZE, rest)
